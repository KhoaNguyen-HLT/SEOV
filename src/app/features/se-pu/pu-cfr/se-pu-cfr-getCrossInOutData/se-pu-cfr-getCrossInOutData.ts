...
@Component({
  selector: 'app-se-pu-cfr-getCrossInOutData',
  standalone: true,
  imports: [FormsModule,
    NzFormModule,
    NzGridModule,
  ...
  templateUrl: './se-pu-cfr-getCrossInOutData.html',
  styleUrls: ['./se-pu-cfr-getCrossInOutData.css']
})
export class sePuCfrGetCrossInOutDataComponent {
  form!: FormGroup;
  user = '';
  msType = '';
  program = true;
  ...
  upload(): void {
    ...
    if (reportName == '15') {
      this.SePuService.getCrossInOutData(formData, month, reportName).subscribe({
        next: (response) => {
  ...
  checkExistedData() {
    ...
    this.SePuService.checkExistedCrossInOutData(formData,month, reportName).subscribe({
      next: (response) => {
        console.log(response);
