...
@Component({
  selector: 'app-cfr-getCrossIvtData',
  standalone: true,
  imports: [FormsModule,
    NzFormModule,
    NzGridModule,
  ...
  templateUrl: './se-pu-cfr-getCrossIvtData.html',
  styleUrls: ['./se-pu-cfr-getCrossIvtData.css']
})
export class sePuCfrGetCrossIvtDataComponent {
  form!: FormGroup;
  user = '';
  msType = '';
  program = true;
  ...
  upload(): void {
    ...
    if (reportName == '15') {
      this.SePuService.getCrossIvtData(formData, month, reportName).subscribe({
        next: (response) => {
  ...
  checkExistedData() {
    ...
    this.SePuService.checkExistedCrossIvtData(formData,month, reportName).subscribe({
      next: (response) => {
        console.log(response);
