...
@Component({
  selector: 'app-se-pu-dpm-get-data',
  standalone: true,
  imports: [
    FormsModule,
    NzFormModule,
  ...
  templateUrl: './se-pu-dpm-getData.html',
  styleUrls: ['./se-pu-dpm-getData.css'],
})
export class sePuDpmGetDataComponent {
  form!: FormGroup;
  user = '';
  msType = '';
  program = true;
  ...
  upload(): void {
    ...
    const dataType = this.form.value.dataType;
    ...
    this.SePuService.getGscmData(formData, dataType).subscribe({
      next: (response) => {
